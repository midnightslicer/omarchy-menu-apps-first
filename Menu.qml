...
Item {
  ...
  // The host hands that engine only to plugins it considers entitled to it,
  // and this menu is handed null, which left the Apps submenu and every app
  // row in search empty. DesktopEntries is the same source the engine reads
  // and is available to any plugin, so the rows come from it directly
  // whenever the injected engine is missing. What stays behind with the
  // engine: hidden-entry filtering beyond NoDisplay, the icon rescan for
  // packages that land after the shell starts, and the launch OSD.
  readonly property bool localApps: !root.appLibrary

  function appEntries() {
    if (root.appLibrary) return root.appLibrary.sortedEntries("")
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      if (!root.appEntryName(entry)) continue
      rows.push({ entry: entry })
    }

    rows.sort(function(a, b) {
      var aName = root.appEntryName(a.entry).toLowerCase()
      var bName = root.appEntryName(b.entry).toLowerCase()
  ...
  function appEntryName(entry) {
    if (root.appLibrary) return root.appLibrary.entryName(entry)
    return String((entry && entry.name) || (entry && entry.id) || "")
  }

  function appEntrySubtext(entry) {
    if (root.appLibrary) return root.appLibrary.entrySubtext(entry)
  ...
  // The engine's launch path, kept verbatim: a scope under app-graphical.slice
  // so the app does not inherit wayland-wm@.service, with gtk-launch resolving
  // the entry so ids with spaces work. The .desktop suffix has to stay or ids
  // like org.telegram.desktop fail to resolve.
  function appLaunch(appId, label) {
  ...
  function appRemove(appId, label) {
  ...
  function rebuildDisplay() {
    ...
    if (query) {
      ...
      for (var i = 0; i < root.itemOrder.length; i++) {
        ...
        // Installed applications are their own group above everything else.
        // Typing the first letters of an app must never land on an installer
        // or a submenu that happens to score better -- `mus` used to select
        // Install > Muse Code ahead of YouTube Music. The installer is still
        // there, one group down.
        if (entry.kind === "app") appRows.push(row)
        else if (entry.parent === active) currentRows.push(row)
      ...
      appRows.sort(searchSort)
      currentRows.sort(searchSort)
      drilldownRows.sort(searchSort)

      // A section name marks where a divider is drawn, so the first group
      // that has rows carries none and every group under it gets its own.
      var groups = [appRows, currentRows, drilldownRows]
      var sections = ["app", "current", "drilldown"]
      for (var g = 0; g < groups.length; g++) {
        if (groups[g].length === 0) continue
        if (rows.length > 0) {
          root.searchDivider = true
          for (var d = 0; d < groups[g].length; d++) groups[g][d].section = sections[g]
        }
  ...
  function confirmDelete() {
    ...
    if (!target) return
    root.cancel()
    root.appRemove(target.appId, target.label)
  }
  ...
  // DesktopEntries is live: an install or an uninstall changes it while the
  // shell runs, so the local rows follow it the way appsChanged does above.
  Connections {
    target: root.localApps ? DesktopEntries.applications : null
